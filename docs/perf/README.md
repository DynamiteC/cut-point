...
| Soak (recorded 1-min run; target 30-min) | Memory growth: 0.9% | PASS (threshold: 20%) |
...
The soak test is a scoped smoke-soak, not a production soak claim. The committed trace is a 1-minute run (`make soak-test-short` defaults to 30 minutes; run it for the full window to reproduce). A true production soak would run 12-24h under realistic load. This catches obvious leaks (unclosed connections, subprocess handle accumulation) but cannot detect slow leaks that manifest only over hours.

## Demo Video Artifact

The most impactful visuals for the demo video:
1. `load-chart.png`: latency stays flat through 100 concurrent in this local run (indicative, not a production guarantee)

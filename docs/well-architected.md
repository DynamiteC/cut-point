...
| | |
|---|---|
| The model is not trusted with measurements | It is not in the analytics path at all. It does produce two judgements of its own finding, `severity` (1-5) and `confidence` (0-1). Only `severity` reaches the report: `Diagnosis` carries `confidence`, but `CliffFinding` has no such field, so the reporter drops it. `severity` is the model's opinion of a cliff the database found, never a measurement of one, and it also selects the recut action. Step 1 reads cliffs, funnel and retention from ClickHouse directly. This is the result of a measurement, not a preference: as an `LlmAgent` that step reported a cliff at second 2 that does not exist and missed all three that do, so it was replaced. The comparison is kept in `validator.validate()` and its tests as the evidence. |
